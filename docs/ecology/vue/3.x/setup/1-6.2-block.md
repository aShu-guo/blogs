...
| 源码片段                           | 逻辑拆解                                      |
|--------------------------------|-------------------------------------------|
| `isBlockTreeEnabled > 0`       | 只有启用 Block 追踪时才会收集                        |
| `currentBlock && !isBlockNode` | 必须存在父 Block，且自身不是 Block 根                  |
| `vnode.patchFlag > 0`          | 有动态标记的节点会被收集                               |
| `shapeFlag & COMPONENT`        | 组件节点始终被收集（即便 patchFlag 为 0）                 |
| `patchFlag !== NEED_HYDRATION` | 仅 hydration 标记的节点不会被视为动态                    |

```typescript
// packages/runtime-core/src/vnode.ts（简化摘录）
if (
...
- 组件节点必须始终参与追踪，确保正确挂载/卸载
- 仅 hydration 用途的节点不会触发动态追踪，避免无意义的 patch
- Block 追踪通过全局状态控制，可在 v-once 等场景临时关闭
