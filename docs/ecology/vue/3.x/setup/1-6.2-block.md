...
Block 机制，或者更准确地说是静态提升（Static Hoisting）和补丁标志（Patch Flags）的应用，旨在解决以下问题：

1. 减少不必要的 VNode 比较： 在 VNode 上标记 Patch Flags，告诉 runtime 哪些部分是动态的，只比较和更新那些有标记的动态部分。
2. 扁平化 VNode 树： 将动态节点收集到一个名为 Block 的数组中，使得 runtime 可以直接遍历这个扁平的数组来更新，而不需要递归地遍历整个 VNode 树。

Block 机制解决的核心问题是：**如何在更新时只 diff 真正变化的节点，而跳过静态节点**。
...
:::warning
我之前理解的是Block机制用于优化state变化时全量render的场景

其实并不是，而是处理state发生变化之前vue2中的全量diff的场景
:::
...
const render = (_ctx) => {
  return (
    ...
    _createBlock(
      ...
      {
        onClick:
          _cache[0] || (_cache[0] = (...args) => _ctx.handleClick(...args)),
      },
      'Click',
    )
  );
