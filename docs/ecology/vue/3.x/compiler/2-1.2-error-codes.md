...
// 现在 errors 数组包含所有编译错误：
// [
//   { code: X_MISSING_ATTRIBUTE_VALUE, message: '...', loc: {...} },
//   { code: X_MISSING_INTERPOLATION_END, message: '...', loc: {...} },
//   { code: X_V_IF_NO_EXPRESSION, message: '...', loc: {...} },
//   { code: X_V_FOR_MALFORMED_EXPRESSION, message: '...', loc: {...} }
// ]
```
...
1. 第一个 `\{\{ message1` 未闭合
2. 遇到第二个 `\{\{`，触发：`X_MISSING_INTERPOLATION_END`
3. 尝试恢复，可能分成两个独立的插值
