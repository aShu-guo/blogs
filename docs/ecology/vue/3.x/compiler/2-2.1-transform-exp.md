...
- **transformExpression 主函数**：`packages/compiler-core/src/transforms/transformExpression.ts:49` - NodeTransform 节点转换
- **processExpression**：`packages/compiler-core/src/transforms/transformExpression.ts` - 表达式处理核心逻辑
- **rewriteIdentifier**：`packages/compiler-core/src/transforms/transformExpression.ts` - 标识符重写逻辑（Inline/非Inline模式）
- **transformExpression.ts 文件**：`packages/compiler-core/src/transforms/transformExpression.ts` (420 lines) - 完整实现
...
详见 [inline 模式](./2-2.1-transform-exp-inline.md) 文档。

#### 非 Inline 模式

详见 [inline 模式](./2-2.1-transform-exp-inline.md) 文档中的**非 Inline 模式**部分。
