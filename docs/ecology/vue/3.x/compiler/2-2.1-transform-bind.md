...
const template = `
  <div
    :id              <!-- 同名简写 -->
    :aria-label.camel="label"  <!-- 驼峰修饰符 -->
    :[attrName].prop="value"   <!-- 动态 + prop 修饰符 -->
  ></div>
`
...
AST = {
  ...
  tag: 'div',
  props: [
    { type: DIRECTIVE, name: 'bind', arg: 'id', exp: null },
    { type: DIRECTIVE, name: 'bind', arg: 'aria-label', exp: 'label', modifiers: ['camel'] },
    { type: DIRECTIVE, name: 'bind', arg: '[attrName]', exp: 'value', modifiers: ['prop'] }
  ]
}
...
// └──────────────────────────────┘
// 处理第 1 个指令：:id
props[0].exp = { content: 'id', isConstant: false }
// 结果：:id="id"

// ┌──────────────────────────────┐
// │ transformBind                │
// └──────────────────────────────┘
// 处理第 2 个指令：:aria-label.camel="label"
① 识别 .camel 修饰符
② 驼峰转换 'aria-label' → 'ariaLabel'

// 处理第 3 个指令：:[attrName].prop="value"
① 识别为动态参数 [attrName]
② 添加默认值 → (attrName) || ""
③ 识别 .prop 修饰符
④ 注入前缀 → '.' + (attrName) || ""
...
props = [
  { key: 'id', value: 'id' },
  { key: 'ariaLabel', value: 'label' },
  { key: '.${(attrName) || ""}', value: 'value' }
]
...
_createVNode('div', {
  id: _ctx.id,
  ariaLabel: _ctx.label,
  ['.${((' + _ctx.attrName + ') || \"\")}']: _ctx.value
})
```
