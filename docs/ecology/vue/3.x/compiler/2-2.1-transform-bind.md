...
- **transformVBindShorthand**：`packages/compiler-core/src/transforms/transformVBindShorthand.ts:11` - 处理同名简写（`:prop` 无表达式）
- **transformBind**：`packages/compiler-core/src/transforms/vBind.ts:15` - DirectiveTransform 指令转换
- **vBind.ts 文件**：`packages/compiler-core/src/transforms/vBind.ts` (86 lines) - v-bind 指令完整实现
