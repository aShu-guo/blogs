...
- **transformIf 主函数**：`packages/compiler-core/src/transforms/vIf.ts:38` - StructuralNodeTransform 结构化转换
- **vIf.ts 文件**：`packages/compiler-core/src/transforms/vIf.ts` (350 lines) - v-if/v-else-if/v-else 完整实现
- **IfNode 和 IfBranchNode**：`packages/compiler-core/src/ast.ts:279-291` - 条件节点数据结构定义
